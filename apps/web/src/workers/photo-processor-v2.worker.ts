...
async function process(id: string, file: File): Promise<void> {
  ...
  try {
    ...
    for (const [kind, maxEdge, webpQuality, jpegQuality] of specs) {
      ...
      post({ id, type: "variant", variant });
    }
    post({ id, type: "complete" });
  } finally {
    bitmap.close();
  }
}
...
worker.addEventListener("message", (event: MessageEvent<PhotoWorkerRequest>) => {
  const { id, protocolVersion, file } = event.data;
  if (protocolVersion !== PHOTO_WORKER_PROTOCOL_VERSION) {
    post({
      id,
      type: "error",
      message: "照片处理组件版本已更新，请刷新页面后重试",
    });
    return;
  }
  void process(id, file).catch((error: unknown) => {
    post({
      id,
      type: "error",
