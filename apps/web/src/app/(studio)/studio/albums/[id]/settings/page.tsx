import type { AlbumStatistics, AlbumView } from "@photostream/contracts";
...
export default async function AlbumSettingsPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const [album, statistics] = await Promise.all([
  ...
  return (
    <section aria-labelledby="settings-heading" className="flex flex-col gap-4">
      ...
      <AlbumSettings initialAlbum={album} statistics={statistics} />
    </section>
  );
}
