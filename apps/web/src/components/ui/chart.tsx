...
function ChartTooltipContent({
  ...
  return (
    <div
      className={cn(
        "grid min-w-36 items-start gap-1.5 rounded-lg border bg-background px-2.5 py-2 text-xs shadow-xl",
        className,
      )}
    >
    ...
    >
      {payload
        ...
        .map((item, index) => {
          ...
          const itemConfig = getPayloadConfigFromPayload(config, item, key);
          return (
            <div className="flex items-center gap-1.5 [&>svg]:size-3" key={`${key}-${index}`}>
