...
状态：本地可执行基线；人脸控制面已实现；云端与生产执行需单独授权
更新日期：2026-09-02
...
备份只包含香港控制面元数据。照片正文仍位于北京私有 OSS，不通过备份脚本或香港 HTTP 服务传输。
...
3. 无法证明人物范围完整时，关闭该相册人脸搜索，分页 `BatchDeleteFileMeta` 清空元数据，再 `DeleteDataset` 并读回两者不存在；不能用“结果已隐藏”代替云端删除。
4. 删除失败保持 `deleting/failed`、指数退避并在后台醒目标示；完成条件是供应商读回不存在、没有待处理索引事件且排除门禁不会重建。
5. 只保留不含人物、照片、聚类、相似度或供应商 URI 的操作审计；按学校流程回复处理结果。

### 9.5 停用与资源清理

永久停用顺序固定为：关闭全局/相册开关 → 取消未开始任务 → 删除参考照和短期结果 → 分页清空全部 IMM 文件元数据 → 删除所有 Dataset 并读回 → 禁用 EventBridge 规则 → 解绑两个 OSS Bucket → 删除临时 Bucket 对象/Bucket → 撤销人脸 RAM 凭证 → 导出并确认最终账单。任何一步失败都保留可重试状态，不提前宣称完成。
