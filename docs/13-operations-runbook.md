...
本手册覆盖应用密钥轮换、孤立上传清理、内容安全事件、CDN 域名迁移和人脸索引事件响应。它不授权部署、DNS、OSS/CDN/IMM/EventBridge 或生产数据操作；执行者必须先记录目标环境、批准人、时间、回滚点和输出位置。

PhotoStream 当前不提供应用层 PostgreSQL 备份、恢复、异地备份或自动备份调度工具；相关脚本、环境变量和部署模板已从当前代码基线移除。
...
1. 在维护窗口内把旧 current 三项复制为 previous，并配置全新的 current 数据密钥、搜索密钥和版本。
...
域名迁移不得改写数据库 object key、代理照片经过香港、启用未批准的云端图片处理或顺带创建其他阿里云产品；人脸资源只能按独立授权和第 6 节操作，不能借 CDN 迁移顺带变更。
