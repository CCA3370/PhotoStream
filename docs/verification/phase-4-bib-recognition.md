...
- 状态机与权限：`suggested/confirmed/rejected/needs_review` 标签状态、独立 OCR 活动状态、照片级 `pending/numbers_confirmed/no_number_confirmed/needs_review`；上传者只操作自己照片，审核员/管理员操作全部，批量动作仅审核员/管理员可用；
- 人工门禁：OCR 候选、无候选、失败和全拒绝均不自动进入搜索或变成“无号码”；确认无号码与确认号码互斥，但保留现有及迟到 OCR 候选供查看/重新选择，候选不会进入搜索且不会覆盖人工结论；添加号码撤销无号码，删除最后确认号码回到待复核；
...
- **Unverified**：号码数据/搜索密钥轮换、备份隔离恢复与旧/新双版本查询；这些属于阶段 5 加固，当前只实现单当前版本和持久 `keyVersion`；
- **Unverified**：学校对号码用途、隐私说明、授权样本、删除流程与复核责任人的确认；所有自动化使用非人物合成图和虚构号码；
- 后续安全性能加固与部署不由本记录声明完成；没有创建或修改 DNS、OSS、CDN、香港主机、CI 发布或任何生产资源。
