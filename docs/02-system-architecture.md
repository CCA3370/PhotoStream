...
| 香港应用主机 | 账号、相册、分类、对象 key、宽高、大小、状态、加密号码标签/框坐标、年级/班级派生属性、无号码复核结论、匿名统计、审计 | 照片正文、缩略图、BlurHash、GPS、原始文件名、号码明文日志 |
| 杭州 OSS | 原图、WebP 派生图、静态构建/OCR 模型资源、加密数据库备份 | 公开读 ACL、云端转换/OCR 结果 |
...
- 公共相册保持 Server Component 主体，分类、搜索、SSE 增量和灯箱分别作为最小 Client Island；工作台组件和 OCR 不得进入公共相册初始客户端图。
...
- 不允许在没有评审的情况下加入 Redis、消息队列、RDS、函数计算、云端媒体处理或第三方分析服务。
