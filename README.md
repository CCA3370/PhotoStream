...
2026-09-17 已批准[管理端本地智能修图计划](docs/16-local-photo-editing.md)：A 类确定性智能优化与 B 类端侧 AI 降噪/轻度清晰化均只在管理端浏览器本机执行，不使用云 AI 或服务器 GPU；编辑采用独立 revision、不可变 OSS 对象和 active revision 回滚。该能力当前只有实施方案，尚未开始代码实现、模型 PoC 或生产验收。
...
- 管理端本地智能修图只允许浏览器端确定性处理和自托管模型端侧推理；不调用云 AI、不要求服务器 GPU、不覆盖原图，编辑结果以独立 revision 保存。
...
- 管理端本地智能修图在 Phase 0/1 代码、真实样片与浏览器设备门禁完成前保持“仅计划”；B 的任何模型在许可证、WebGPU、性能和视觉真实性 PoC 通过前不得作为生产能力启用；
