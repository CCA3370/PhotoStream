...
“中学部影像直播”是面向北航实验学校中学部暨北航实验学校分校的校内照片直播平台。摄影、审核和管理人员通过浏览器完成照片上传与发布，师生和家长通过口令相册在微信或现代浏览器中查看活动影像。

## 当前状态

阶段 0 文档基线已经提交。用户于 2026-08-27 明确授权进入编码阶段；当前已完成阶段 1–5 的本地实现，包括浏览器照片派生与直传、口令公共相册、SSE/轮询、PWA 离线壳与恢复队列、批量审核与筛选、两类下载策略、成员与审计、持久删除/孤立上传清理、结构化号码规则、本地 OCR、人工确认、精确号码/年级班级筛选、strict CSP 和号码密钥轮换。产品边界现已收缩为仅照片，其他媒体类型不进入契约、数据库或路线图。
...
2026-09-01 已按 ADR-012 完成人脸候选找图的本地代码实现：口令相册可在学校完成授权门禁后使用北京阿里云 IMM 和 EventBridge，让观众提交一张参考照筛选可能包含同一人物的已发布照片。全局开关默认关闭；没有开通云资源或处理人脸样本，云端、真实数据库、准确率、设备与生产运行证据保持 **Unverified**。
...
2026-09-17 已批准[管理端本地智能修图计划](docs/16-local-photo-editing.md)：A 类确定性智能优化与 B 类端侧 AI 降噪/轻度清晰化均只在管理端浏览器本机执行，不使用云 AI 或服务器 GPU；编辑采用独立 revision、不可变 OSS 对象和 active revision 回滚。该能力按正常生产路径直接实现，不设置独立模型 PoC/晋级阶段；PhotoStream 当前尚未正式对外投产，因此模型、参数和性能直接在当前生产环境中验证、调整和迭代。只要当前设备仍可访问上传时保存的本地完整源，修图禁止重新从 CDN/OSS 获取原图；有 active edit 时，观众端“原图下载”解析到该 revision 的全分辨率修图版本，真正上传原图仅保留给管理端恢复和重新编辑。
...
- 香港 2C2G Linux 主机承载主站、API 和 PostgreSQL；北京 OSS 与中国内地 CDN 承载全部媒体内容。
...
- 管理端本地智能修图只允许浏览器端确定性处理和自托管模型端侧推理；不调用云 AI、不要求服务器 GPU、不覆盖真正上传原图，编辑结果以独立 revision 保存。
- 修图数据源严格本地优先：当前设备能访问上传时保存的完整本地源时，不得为了修图再次从 CDN/OSS 下载原图；只有本地源不存在时才允许远端回退。
- 有 active edit 时，观众的 480/960/1920 与“原图下载”都解析到同一个修图 revision；“原图下载”返回该 revision 的全分辨率 `photo_download`，真正上传原图只供管理端恢复/重新编辑。
...
14. [运维与事件响应手册](docs/13-operations-runbook.md)
15. [Debian 13 完整部署操作手册](docs/15-debian13-deployment.md)
16. [管理端本地智能修图](docs/16-local-photo-editing.md)
17. [参考资料](docs/references.md)
18. [架构决策记录](docs/adr/README.md)
...
- 管理端本地智能修图不设置独立 PoC/生产晋级门禁；按 `docs/16-local-photo-editing.md` 直接接入正常生产代码路径，并在当前未正式对外投产的生产环境中完成模型、参数、性能、兼容性和视觉效果调整；
